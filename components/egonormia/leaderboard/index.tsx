        ...
        <div className="bg-gray-100 dark:bg-gray-900 p-4 rounded-lg text-sm text-gray-700 dark:text-gray-300">
          ...
          <ul className="space-y-2">
            <li className="flex items-center gap-2">
              <span className="inline-block w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: modalityColors['Blind'] }}></span>
              <strong>Blind:</strong> Models receive only the questions with no visual input
            </li>
            <li className="flex items-center gap-2">
              <span className="inline-block w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: modalityColors['Pipeline'] }}></span>
              <strong>Pipeline:</strong> Models receive text-only descriptions of the scene (generated by Gemini 1.5 Flash)
            </li>
            <li className="flex items-center gap-2">
              <span className="inline-block w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: modalityColors['Video'] }}></span>
              <strong>Video:</strong> Models receive both video input (1 fps, concatenated into a single image) and questions
            </li>
          </ul>
        </div>
        ...
        <div className="flex flex-wrap gap-2 justify-center">
          ...
          {Object.keys(modalityColors).map(modality => (
            <button
              ...
              style={{ backgroundColor: filterModality === modality ? modalityColors[modality] : undefined }}
            >
              {modality}
            </button>
          ))}
          ...
          <table className="w-full caption-bottom text-xs sm:text-sm md:text-base table-fixed">
            ...
            <tbody className="[&_tr:last-child]:border-0">
              {sortedEntries.map((entry, index) => (
                <tr key={`${entry.model}-${entry.modality}-${index}`} className="border-b border-gray-200 dark:border-gray-700 transition-colors hover:bg-gray-100 dark:hover:bg-gray-800 data-[state=selected]:bg-gray-100 dark:data-[state=selected]:bg-gray-800">
                  ...
                  <td className="p-1 sm:p-2 align-middle text-right dark:text-white font-mono">{entry.both.toFixed(1)}</td>
                  <td className="p-1 sm:p-2 align-middle text-right dark:text-white font-mono">{entry.act.toFixed(1)}</td>
                  <td className="p-1 sm:p-2 align-middle text-right dark:text-white font-mono">{entry.jus.toFixed(1)}</td>
                  <td className="p-1 sm:p-2 align-middle text-right dark:text-white font-mono">{entry.sens.toFixed(1)}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="w-full max-w-full overflow-x-hidden">
      <div className="bg-[#1a1a1a] p-4 sm:p-6 rounded-t-lg">
        <div className="text-center mb-6 sm:mb-8">
          <h1 className="text-3xl sm:text-4xl md:text-6xl font-bold mb-4 sm:mb-6 md:mb-8 tracking-wider text-white">EGONORMIA LEADERBOARD</h1>
          
          <p className="text-xl sm:text-2xl md:text-3xl mb-6 text-white font-light">Benchmarking Physical Social Norm Understanding of VLM</p>
